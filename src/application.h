...
public:
	...
	{
		auto [it, inserted] = _documents.emplace(std::make_unique<Document>());
		assert(inserted);
		return *it;
	}
	...
	{
		std::size_t removed = _documents.erase(document);
		if (removed == 0)
			return false;
		return true;
	}
...
private:
	std::unordered_set<std::unique_ptr<Document> > _documents;
};
